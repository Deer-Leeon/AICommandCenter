...
// ── Waiting-for-browser overlay (Electron only) ───────────────────────────────

function AwaitingBrowserScreen({ onCancel }: { onCancel: () => void }) {
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(124,106,255,0.3) 0%, rgba(61,232,176,0.15) 100%)',
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {[0, 1, 2].map(i => (
            <div key={i} style={{
              ...
              animation: `login-bounce 1.2s ease-in-out ${i * 0.2}s infinite`,
            }} />
          ))}
        </div>
...
export default function LoginPage() {
  const { signInWithGoogle, awaitingBrowser, cancelBrowserAuth } = useAuth();
  ...
  if (awaitingBrowser) {
    return <AwaitingBrowserScreen onCancel={cancelBrowserAuth} />;
  }
