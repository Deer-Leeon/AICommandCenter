      ...
      try {
        const res = await apiFetch('/api/ai', {
          ...
          body: JSON.stringify({
            ...
            // Tell the AI which Slack channel is currently open in the widget
            activeSlackChannel: localStorage.getItem('nexus-slack-channel') ?? '',
          }),
        });
        ...
        if (data.intent === 'CREATE_CALENDAR_EVENT') {
          if (data.result) {
            ...
            // Register as pending so refetches don't wipe it during propagation
            if (newEvent.id) addPendingCalendarEventId(newEvent.id);
            // Optimistic add — appears in the widget immediately
            setCalendarEvents([...calendarEvents, newEvent]);
          }
          // Confirming refetches: 4 s gives Google Calendar time to propagate,
          // 10 s is a safety-net in case the first still misses it
          ...
          // Refetch from Supabase so the new todo (with real ID) appears immediately
          triggerTodosRefetch();
          flashWidget('todo');
        }
      } catch (err) {
